...
class Director(object):
    def __init__(self, hw_registry, setup_registry):
        ...
        for mrc in self.setup_registry.mrcs:
            self._setup_mrc_added(mrc)
            mrc.device_added.connect(partial(self._setup_mrc_device_added, mrc))
            mrc.device_removed.connect(partial(self._setup_mrc_device_removed, mrc))

        hw_registry.mrc_added.connect(self._hw_mrc_added)
        hw_registry.mrc_removed.connect(self._hw_mrc_removed)

        setup_registry.mrc_added.connect(self._setup_mrc_added)
        setup_registry.mrc_removed.connect(self._setup_mrc_removed)
    ...
    def _setup_mrc_device_added(self, mrc, device):
        app_mrc = self.registry.get_mrc(mrc.url)
        app_device = app_mrc.get_device(device.bus, device.address)
        if app_device is None:
            app_device = Device(device.bus, device.address, device.idc)
    ...
    def _setup_mrc_device_removed(self, mrc, device):
        app_mrc = self.registry.get_mrc(mrc.url)
        app_device = app_mrc.get_device(device.bus, device.address)
        app_device.cfg = None
