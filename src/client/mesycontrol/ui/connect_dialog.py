...
class ConnectDialog(QtGui.QDialog):
    def __init__(self, context, parent=None):
        ...
        self.ui = util.loadUi(":/ui/connect_dialog.ui", self)
        self.ui.buttonBox.button(QtGui.QDialogButtonBox.Ok).setEnabled(False)
        self.ui.combo_serial_port.setValidator(QtGui.QRegExpValidator(QtCore.QRegExp('.+'), None))
        ...
        for port in util.list_serial_ports():
            self.ui.combo_serial_port.addItem(port)
    ...
    def _validate_inputs(self):
        ...
        self.ui.buttonBox.button(QtGui.QDialogButtonBox.Ok).setEnabled(is_ok)
    ...
    def accept(self):
        ...
        if idx == 0:
            self.connection_config.serial_device    = self.ui.combo_serial_port.currentText()
            ...
            self.connection_config.tcp_host = self.ui.le_tcp_host.text()
            self.connection_config.tcp_port = self.ui.spin_tcp_port.value()
        elif idx == 2:
            self.connection_config.mesycontrol_host = self.ui.le_mesycontrol_host.text()
            self.connection_config.mesycontrol_port = self.ui.spin_mesycontrol_port.value()
